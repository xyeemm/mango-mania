...
const features = [
  ...
  { icon: Award, label: "Curated selection", sub: "Sourced from trusted growers" },
];
...
const heroImages = [
  MANGO_PRODUCTS.find((p) => p.id === "alphonso"),
  MANGO_PRODUCTS.find((p) => p.id === "kesar"),
  MANGO_PRODUCTS.find((p) => p.id === "mixed-box"),
].filter(Boolean) as (typeof MANGO_PRODUCTS)[number][];
